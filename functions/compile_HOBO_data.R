...
compile_HOBO_data <- function(path.HOBO, area.name, serial_table, deployment_range){
  ...
  for(i in 1:length(dat.files)) {
    ...
    # use serial number to identify the variable and depth (from serial_table)
    variable_depth <- serial_table %>% 
    ...
    hobo.i <- hobo.i %>%
      ...
      add_row(INDEX = as.character(-4), DATE= deployment_range, TEMPERATURE = deployment_range, .before = 1) 
  ...
  # extract the deployment date from deployment_range
  file.date <- separate(data = data.frame(deployment_range), 
